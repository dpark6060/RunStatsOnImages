...
def CompileNii(OriginalFile,Save):
    ...
    txtDir=os.path.join(BaseDir,'{}_Work'.format(subID),'WorkFiles','Filt')
    
    for z in range(lz):
        zstack=np.zeros((lx*ly,lt))
        ztxts=os.path.join(txtDir,'Filt_{}_{}_*.txt'.format('Image',z))
        ...
        if not len(ind)==0:
            ...
            for ii in range(nzchunks):
                print(os.path.join(txtDir,'Filt_{}_{}_{}.txt'.format('Image',z,ii)))
                newdata=np.loadtxt(os.path.join(txtDir,'Filt_{}_{}_{}.txt'.format('Image',z,ii)))
    ...
    savefile=os.path.join(BaseDir,'{}_SR.nii.gz'.format(subID))
    nb.loadsave.save(img,Save)
